variable "project_id" {
  description = "Globally unique project ID for this environment (e.g. acme-platform-prod)."
  type        = string
}
...
variable "project_name" {
  ...
  type        = string
  default     = "ACME Platform production"
}
...
variable "node_locations" {
  description = "Explicit zones for node pools to guarantee multi-zone spread in production."
  type        = list(string)
...
# --- Sizing / posture knobs (production: hardened, HA, fully enforced) ---
