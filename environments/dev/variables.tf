variable "project_id" {
  description = "Globally unique project ID for this environment (e.g. acme-platform-dev)."
  type        = string
}
...
# --- Sizing / posture knobs (dev defaults are intentionally small & cheap) ---

variable "enable_private_endpoint" {
  description = "Whether the GKE control plane endpoint is private-only."
  type        = bool
  default     = false # dev allows a public (authorized-network-gated) endpoint for easier access
}
